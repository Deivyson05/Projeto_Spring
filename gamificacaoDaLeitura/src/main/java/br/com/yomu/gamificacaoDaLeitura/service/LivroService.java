...
public class LivroService {
    ...
    public Livro atualizar(UUID id, Livro livroAtualizado) {
        ...
        if (livroAtualizado.getDescricao() != null) {
            livro.setDescricao(livroAtualizado.getDescricao());
        }
        if (livroAtualizado.getCapa() != null) {
            livro.setCapa(livroAtualizado.getCapa());
        }
        if (livroAtualizado.getFinalizado() != null) {
            livro.setFinalizado(livroAtualizado.getFinalizado());
        }
    ...
    private void validarLivro(Livro livro) {
        ...
        } else if (livro.getTipoRegistro() == TipoRegistro.CAPITULO) {
            if (livro.getNumeroCapitulos() == null || livro.getNumeroCapitulos() <= 0) {
